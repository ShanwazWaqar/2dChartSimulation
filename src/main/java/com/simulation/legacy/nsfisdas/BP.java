...
public class BP
{
 ...
 private void openFiles() {
  try {
   File pcData = new ClassPathResource("legacy-resources/nsfisdas/" + PCFile).getFile();
   File coeffsData = new ClassPathResource("legacy-resources/nsfisdas/" + CoeffsFile).getFile();

   pcFile = new RandomAccessFile(pcData, "r");
   coeffsFile = new RandomAccessFile(coeffsData, "r");

  } catch (IOException e) {
   System.err.println("Error while opening files: " + e.getMessage());
   // Optional: throw RuntimeException or handle error
  }
 }
