...
/**
 * Bean class for corrosion analysis parameters and calculations
 */
@Component
	...
	public CorrosionBean() {
		...
		public String getInitiationLife(){
			...
			try{
				...
				if (Double.isInfinite(inLife) || Double.isNaN(inLife) || inLife < 0) inLife = 0.0;
		...
		public String getcrackPropagation(){
			...
			try{
				...
				if (af_final == null || af_final.isEmpty()) af_final = "0.1";
				if (apc_final == null || apc_final.isEmpty()) apc_final = "0.01";
				...
				if (C1 == 0.0) C1 = 1e-10;
				if (C2 == 0.0) C2 = 1e-10;
				if ((M1/2)-1 == 0.0) M1 = 2.01; // avoid zero denominator
				if ((M2/2)-1 == 0.0) M2 = 2.01;
				...
				if (Double.isInfinite(crackNp) || Double.isNaN(crackNp) || crackNp < 0) crackNp = 0.0;
				NumberFormat formatter = new DecimalFormat("0.0000E0");
